...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    const { jobId, title, description, requirements, academicLevel, status, visibility } = requestBody;
    ...
    // Check if user has any of the required roles to update jobs
    if (!hasAnyRole(userRoles, ['Admin', 'Professor', 'LabAssistant'])) {
      return createErrorResponse(403, 'Unauthorized. You need Admin, Professor, or LabAssistant role to update jobs.');
    }
    ...
    if (!canManageJob(userId, userRoles, job, lab)) {
      return createErrorResponse(403, 'You do not have permission to update this job');
    }
    ...
    if (academicLevel !== undefined) {
      updateExpression += ', academicLevel = :academicLevel';
      expressionAttributeValues[':academicLevel'] = academicLevel;
    }
    ...
    const updatedJobResponse = await client.send(updateItemCommand);
    
    // Return the updated job
    const updatedJob = updatedJobResponse.Attributes 
      ? unmarshall(updatedJobResponse.Attributes)
      : null;
